...
function TreasuryApp({ basePath }: Props): React.ReactElement<Props> {
  ...
  useEffect((): void => {
    if (tipHashTrigger && mountedRef.current) {
      api.query.treasury.tips
        .keys()
        .then(keys => {
          mountedRef.current && setTipHashes(keys.map(key => key.args[0].toHex()));
        })
        .catch(console.error);
    }
    ...
    <main className="treasury--App">
      ...
      <Switch>
        <Route path={`${basePath}/tips`}>
          <Tips hashes={tipHashes} isMember={isMember} members={members} trigger={triggerTipHashes} />
        </Route>
        <Route>
          <Overview isMember={isMember} members={members} />
        </Route>
      </Switch>
    </main>
  );
